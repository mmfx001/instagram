...
import { FaUserCircle, FaTrash, FaEdit, FaPaperPlane, FaStop, FaFileAlt } from 'react-icons/fa';
...
const Messenger = () => {
    ...
    const handleUserClick = (user) => {
        ...
        updatedMessages.forEach(msg => {
            if (msg.sender === user.email && msg.receiver === loggedInUser.email && msg.status === "view") {
                axios.put(`http://localhost:5001/messages/${msg.id}`, { ...msg, status: "view" })
    ...
    const deleteMessage = (messageId) => {
        axios.delete(`http://localhost:5001/messages/${messageId}`)
            .then(() => {
            ...
            >
                {selectedUser ? (
                    <>
                        ...
                        <div className="border p-4 rounded-lg bg-gray-800 flex-grow overflow-y-auto shadow ">
                            {messages.filter(msg =>
                                ...
                                <div key={index} className={`mb-3 flex ${msg.sender === loggedInUser.email ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`relative ${msg.sender === loggedInUser.email ? 'bg-blue-500 text-white' : 'bg-gray-700 text-gray-300'} rounded-2xl py-4 px-4 max-w-xs shadow-md ` } onClick={() => handleMessageClick(msg)}>
                                        ...
                                        {showMessageOptions === msg.id  && (
                                            <div className="absolute z-20 bg-gray-700 p-2 rounded-xl w-48 right-0 top-[-100px] opacity-100 transition-opacity duration-300 flex flex-col">
